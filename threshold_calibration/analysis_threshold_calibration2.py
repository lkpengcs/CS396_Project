...
if __name__ == "__main__":
    ...
    for name in models:
        ...
        df_sens.to_csv(parent_dir / "threshold_calibration" / f"threshold_sensitivity2_{name}.csv", index=False)
        ...
        plt.grid(True)
        plt.savefig(parent_dir / "threshold_calibration" / f"threshold_accuracy_plot2_{name}.png")
        plt.close()
        ...
        plt.grid(True)
        plt.savefig(parent_dir / "threshold_calibration" / f"calibration_curve2_{name}.png")
        plt.close()

        # Compute ECE
        ece_val = calibration_error(y_test, y_prob)
        with open(parent_dir / "threshold_calibration" / f"calibration_metrics2_{name}.txt", "w") as f:
        ...
        for attr in protected_attrs:
            ...
            df_gcal.to_csv(parent_dir / "threshold_calibration" / f"group_calibration2_{name}_{attr}.csv", index=False)

    print("\n✔ Step C complete: saved threshold_sensitivity2 + calibration metrics2 + plots2.")
