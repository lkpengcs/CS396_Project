...
def plot_model_performance():
    ...
    baseline_path = Path("advanced/advanced_results.json")
    ...
    plt.tight_layout()
    plt.savefig("figures/performance_comparison2.png", dpi=300, bbox_inches="tight")
    plt.close()
    print("✓ Saved: figures/performance_comparison2.png")
...
def plot_fairness_gaps():
    ...
    plt.tight_layout()
    plt.savefig("figures/fairness_gaps_by_attr2.png", dpi=300, bbox_inches="tight")
    plt.close()
    ...
    plt.legend(title="Metric", fontsize=10)
    plt.tight_layout()
    plt.savefig("figures/fairness_gaps_by_model2.png", dpi=300, bbox_inches="tight")
    plt.close()
    
    print("✓ Saved: figures/fairness_gaps_by_attr2.png")
    print("✓ Saved: figures/fairness_gaps_by_model2.png")
...
def plot_error_asymmetry():
    ...
    for model in MODELS:
        for attr in PROTECTED_ATTRS:
            file_path = Path(f"error_analysis/{model}/error_asymmetry2_{attr}.csv")
            ...
            plt.savefig(f"figures/error_asymmetry2_{model}_{safe_attr}.png", dpi=300, bbox_inches="tight")
            plt.close()
            print(f"✓ Saved: figures/error_asymmetry2_{model}_{safe_attr}.png")
...
def plot_threshold_sensitivity():
    ...
    for model in MODELS:
        file_path = Path(f"threshold_calibration/threshold_sensitivity2_{model}.csv")
        if not file_path.exists():
            continue
        ...
        plt.tight_layout()
        plt.savefig(f"figures/threshold_sensitivity2_{model}.png", dpi=300, bbox_inches="tight")
        plt.close()
        print(f"✓ Saved: figures/threshold_sensitivity2_{model}.png")
...
def plot_calibration_curves():
    ...
    model_names = []
    for model in MODELS:
        img_path = Path(f"threshold_calibration/calibration_curve2_{model}.png")
        if img_path.exists():
    ...
    plt.tight_layout()
    plt.savefig("figures/calibration_curves_combined2.png", dpi=300, bbox_inches="tight")
    plt.close()
    print("✓ Saved: figures/calibration_curves_combined2.png")
...
def plot_counterfactual_summary():
    ...
    rows = []
    for model in MODELS:
        file_path = Path(f"counterfactual_analysis/cf_summary2_{model}.csv")
        if not file_path.exists():
            continue
    ...
    plt.tight_layout()
    plt.savefig("figures/counterfactual_gap2.png", dpi=300, bbox_inches="tight")
    plt.close()
    print("✓ Saved: figures/counterfactual_gap2.png")
...
def plot_feature_importance():
    ...
    for model in MODELS:
        file_path = Path(f"error_analysis/{model}/feature_importance2.csv")
        if not file_path.exists():
            continue
        ...
        plt.tight_layout()
        plt.savefig(f"figures/feature_importance2_{model}.png", dpi=300, bbox_inches="tight")
        plt.close()
        print(f"✓ Saved: figures/feature_importance2_{model}.png")
...
def plot_calibration_by_group():
    ...
    for model_result in results:
        ...
        if not overall_df.empty:
            ...
            plt.savefig(f"figures/calibration_overall2_{safe_model}.png", dpi=300, bbox_inches="tight")
            plt.close()
            print(f"✓ Saved: figures/calibration_overall2_{safe_model}.png")
