...
if __name__ == "__main__":
    ...
    for name in models:
        ...
        fi_df.to_csv(parent_dir / "error_analysis" / name / f"feature_importance.csv", index=False)

        # Per protected attribute error asymmetry
        for attr in protected_attrs:
            df = group_error_asymmetry(y_test, y_pred, X_test_raw, attr)
            df.to_csv(parent_dir / "error_analysis" / name / f"error_asymmetry_{attr}.csv", index=False)
