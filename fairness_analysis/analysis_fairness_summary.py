...
if __name__ == "__main__":
    ...
    data = load_baseline_results(parent_dir / "baseline" / "baseline_results.json")

    print(f"▶ Building fairness summary table ...")
    fairness_df = build_fairness_summary(data)
    fairness_df.to_csv(parent_dir / "fairness_analysis" / "fairness_summary.csv", index=False)

    # Also save to markdown (good for reports)
    fairness_df.to_markdown(parent_dir / "fairness_analysis" / "fairness_summary.md", index=False)

    print(f"Saved fairness_summary.csv and fairness_summary.md to {parent_dir / 'fairness_analysis'}")

    print(f"▶ Building performance summary ...")
    perf_df = build_performance_summary(data)
    perf_df.to_csv(parent_dir / "fairness_analysis" / "performance_summary.csv", index=False)
    perf_df.to_markdown(parent_dir / "fairness_analysis" / "performance_summary.md", index=False)

    print(f"Saved performance_summary.csv and performance_summary.md to {parent_dir} / fairness_analysis /")
